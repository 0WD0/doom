...
大小还需要和 latex 统一一下，现在基本的功能差不多是实现了
还需要和 [[#latex-resize][LaTeX 预览图像缩放]] 配合一下，设置好统一的初始 svg 大小

#+begin_src emacs-lisp
;;; org-typst-advice.el --- Add Typst support to org-mode formula rendering

;;; Commentary:
;; This advice adds Typst compilation support to org-create-formula-image.
;; For $ delimited formulas when processing-type is 'latex, it first tries
;; to compile with Typst. If successful, uses the SVG output directly.
;; If Typst compilation fails, falls back to the original LaTeX processing.
...
STRING, TOFILE, OPTIONS, BUFFER, and PROCESSING-TYPE are the original arguments."
  (let ((actual-processing-type (or processing-type org-preview-latex-default-process)))
    ;; Check if we should try Typst: processing-type is latex and string is $ delimited
    (if (and (eq actual-processing-type 'dvisvgm)
             (numberp (string-match-p "^\\s-*\\$.*\\$\\s-*$" string)))
        ;; Try Typst compilation first
        (let* ((tmpdir temporary-file-directory)
               ...
               (typst-content (replace-regexp-in-string
                               "^\\s-*\\$\\s-*\\|\\s-*\\$\\s-*$" "" string))
               ;; Get foreground and background colors from options
               ...
               ;; Process colors - handle hex colors and special values
               (fg (cond
                    ((null fg-raw) nil)  ; No color specified
                    ((eq fg-raw 'default) nil)  ; Use default
                    ((stringp fg-raw) fg-raw)  ; Use as-is (supports hex like "#c6d0f5")
                    (t nil)))
               (bg (cond
                    ((null bg-raw) nil)  ; No background specified
                    ((eq bg-raw 'default) nil)  ; Use default
                    ((string= bg-raw "Transparent") "Transparent")
                    ((stringp bg-raw) bg-raw)  ; Use as-is (supports hex like "#303446")
                    (t "Transparent"))))
          (condition-case err
              (progn
                ;; Create Typst file with math content
                (with-temp-file typst-file
                  ...
                  ;; Insert the math content
                  (insert "$ " typst-content " $"))

                ;; Try to compile with Typst
                (let ((exit-code (call-process "typst" nil nil nil
                  ...
                  (if (and (eq exit-code 0) (file-exists-p svg-file))
                      (progn
                        ...
                        tofile)
                    ;; Typst compilation failed, fall back to LaTeX
                    (progn
            ...
            (error
             ;; Error in Typst processing, clean up and fall back to LaTeX
             (when (file-exists-p typst-file) (delete-file typst-file))
             (when (file-exists-p svg-file) (delete-file svg-file))
             (funcall orig-fun string tofile options buffer processing-type))))
      ;; Not a candidate for Typst, use original function
      (funcall orig-fun string tofile options buffer processing-type))))
