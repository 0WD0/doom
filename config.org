...
** 自动为 =:chat:= 标签添加 folded VISIBILITY :ARCHIVE:
...
(defun my/org-slugify (s)
  (let* ((s (downcase (replace-regexp-in-string "[^[:alnum:]]+" "-" s)))
         (s (replace-regexp-in-string "-+" "-" s))
         (s (replace-regexp-in-string "\\`-\\|-\\'" "" s)))
    (if (string-match-p "[[:alnum:]]" s) s "untitled")))

(defun my/org-export-file-name-postprocess (value)
  (my/org-slugify
   (if (org-string-nw-p value)                       ;用户输入了值就规整它
       value
     (or (org-entry-get nil "TITLE")                 ;否则用标题/heading
         (nth 4 (org-heading-components))
         (file-name-base (or (buffer-file-name)      ;或文件名
                             (buffer-name)))
         "untitled"))))

(after! org
  (add-to-list 'org-properties-postprocess-alist
               '("EXPORT_FILE_NAME" . my/org-export-file-name-postprocess)))
#+end_src
...
(after! org
  (require 'org-roam)
  (defun my/org-roam-export-file-name (value)
    "为 EXPORT_FILE_NAME 生成/规整 slug，优先用 org-roam 的规则。"
    (let* ((node (org-roam-node-at-point nil))         ;当前节点或 nil
           (title (or (and node (org-roam-node-title node))
                      ...
                      "")))
      (cond
       ;; 用户输入了值：用 org-roam 的 slugify 规整它
       ((org-string-nw-p value) (org-roam-node-slugify value))
       ;; 没输入值但在 org-roam 节点：用节点 slug（唯一且含 ID 策略）
       (node (org-roam-node-slug node))
       ;; 其他情况：fallback slugify 当前标题
       (t (org-roam-node-slugify title)))))

  (add-to-list 'org-properties-postprocess-alist
               '("EXPORT_FILE_NAME" . my/org-roam-export-file-name)))
               #+end_src
...
我觉得我需要一个批出理脚本，需要加载尽可能多的包？
https://emacs.stackexchange.com/questions/33788/use-package-load-all-defined-packages

TODO: =var:= =cmd:=

TODO: emacs 内置函数（gz 文件里的）链接会失效

TODO: export to org
适合导出为文档，
把本机设置的一些非公认的链接转换为内置的形式，
比如把 =kbd:= 转换回 ~xxx~；
把 =fn:= 链接转换为正常的 https 链接。
...
(after! ox
  ...
  (defun my/helpful-remote-url (sym callable-p)
    (-let* (((buf pos opened) (helpful--definition sym callable-p)))
      (unless (and buf pos) (error "找不到 %s 的源码位置" sym))
      (unwind-protect
          (with-current-buffer buf
            (save-restriction
              (widen)
              ;; 这里传“位置”给 browse-at-remote--file-url，它自己会算行号
              (let* ((start pos)
                     (end   (when callable-p
                              (save-excursion
                                (goto-char pos)
  ...
  (defun my/org-export-helpful-link (type path desc backend)
    ...
    (let* ((sym (intern path))
      ...
      (pcase backend
        ((or `md `gfm `html)
         (if url (format "<a href=\"%s\">%s</a>" url text) text))
        (`latex
...
*** add =#pragma once= for hpp files
#+begin_src elisp
(after! org
  (defun my/org-tangle-hpp-ensure-pragma-once ()
    (when (and buffer-file-name
               (string-suffix-p ".hpp" buffer-file-name))
      (save-excursion
        (goto-char (point-min))
        (unless (looking-at-p "\\s-*#pragma once\\>")
          ;; 删除开头已有的 pragma once，避免重复
          (when (re-search-forward "^#pragma once\\s-*$" nil t)
            (replace-match ""))
          (goto-char (point-min))
          (insert "#pragma once\n\n")
          (save-buffer)))))

  (add-hook 'org-babel-post-tangle-hook #'my/org-tangle-hpp-ensure-pragma-once))
#+end_src
...
#+begin_src emacs-lisp
(after! org
  (dolist (pkg '("amsmath" "amssymb" "mathtools" "mathrsfs" "ctex"))
    (add-to-list 'org-latex-packages-alist `("" ,pkg t)))

  (add-to-list 'org-latex-packages-alist '("most" "tcolorbox" t))
  (add-to-list 'org-latex-packages-alist "\\newenvironment{notes}{\\begin{tcolorbox}[colback=gray!8, colframe=gray!30, boxrule=0.4pt, arc=2pt, left=6pt, right=6pt, top=4pt, bottom=4pt]\\small}{\\end{tcolorbox}}"))
#+end_src
...
#+begin_src emacs-lisp
(after! org
  ;; 任选其一：'dvisvgm (SVG 更清晰) 或 'imagemagick (PNG)
  (setq! org-preview-latex-default-process 'dvisvgm)

  (let ((var 'org-preview-latex-process-alist))
    ;; dvi -> svg (用 lualatex 生成 dvi，避免 fontspec+pdftex 崩)
    (set var (assoc-delete-all 'dvisvgm (symbol-value var)))
    (add-to-list var
                 `(dvisvgm
                   :programs ("lualatex" "dvisvgm")
                   :description "dvi > svg (lualatex)"
                   :message "You need: lualatex and dvisvgm."
    ...
    ;; pdf -> png (如果你更想要 PNG，就把 default-process 设成 'imagemagick)
    (set var (assoc-delete-all 'imagemagick (symbol-value var)))
    (add-to-list var
                 `(imagemagick
                   :programs ("lualatex" "convert")
                   :description "pdf > png (lualatex)"
                   :message "You need: lualatex and imagemagick(convert)."
...
(after! better-jumper
  (add-to-list 'better-jumper-disabled-modes 'org-passwords-mode))
#+end_src
...
(package! ox-hugo
  :recipe (:host github :repo "0WD0/ox-hugo" :nonrecursive t))
#+end_src

#+begin_src elisp
(setq! org-hugo-section "")
(use-package! org-roam-hugo
  :after ox-hugo org-roam)

(setq! org-hugo-auto-update-id-locations nil)
...
(package! paw :recipe (:local-repo "paw"))
#+end_src
...
(after! eww
  ...
  (map! :map eww-mode-map
        ...
        :n "M-l"        #'shrface-links-counsel   ;; or #'shrface-links-helm / #'shrface-links-consult
        :n "M-h"        #'shrface-headline-counsel)) ;; or #'shrface-headline-helm / #'shrface-headline-consult
#+end_src
...
#+begin_src elisp
(setq! majutsu-display-functions '((log . pop-to-buffer)
                                   (op-log . pop-to-buffer)
                                   (diff . pop-to-buffer-same-window)
                                   (message . pop-to-buffer))
       majutsu-executable "jjj")
#+end_src
...
(when (modulep! :lang rust)
  (add-to-list '+lookup-provider-url-alist '("Docs.rs" "https://docs.rs/releases/search?query=%s")))
#+end_src
...
** bibilo 模块设置
#+begin_src emacs-lisp
(setq! citar-bibliography '("~/zotero-all.bib")
...
(setq! gptel-backend
      (gptel-make-openai "AiHubMix"
        ...
        :models '(
                  (gpt-5.1     :capabilities (media tool json url))
...
#+begin_src elisp
(my/org-register-capture-dir
 "/home/disk/Dev/opencode"
 '("i" "opencode dev notes inbox" entry (id "fdaf9709-724b-42ae-b409-ce5702e429a5") "* %?\n%A\n"))
#+end_src
...
#+begin_src elisp
(my/org-register-capture-dir
 "/home/_WD_/.config/emacs/.local/straight/repos/ox-hugo"
 '("i" "ox-hugo notes inbox" entry (id "f7b93568-e35a-449e-9a53-592676a3ae23") "* %?\n%A\n" :prepend t))
#+end_src

* Pdf-Tools
#+begin_src elisp
(defun my/pdf--clipboard-cmd ()
  (cond
   ((executable-find "wl-copy") (list "wl-copy" "--type" "image/png"))
   ((executable-find "xclip")   (list "xclip" "-selection" "clipboard" "-t" "image/png" "-i"))
   (t (user-error "Need wl-copy (Wayland) or xclip (X11)"))))

(defun my/pdf--clipboard-send-bytes (bytes)
  (let* ((cmd (my/pdf--clipboard-cmd))
         (p (make-process :name "pdf->clipboard"
                          :command cmd
                          :connection-type 'pipe
                          :noquery t)))
    (set-process-coding-system p 'binary 'binary)
    (process-send-string p (if (multibyte-string-p bytes) (string-make-unibyte bytes) bytes))
    (process-send-eof p)
    (while (process-live-p p) (accept-process-output p 0.05))))

(defun my/pdf-view-copy-page-image-to-clipboard (&optional force-width)
  "复制当前页 PNG 到系统剪贴板。
  无前缀：直接复制当前显示页的缓存 PNG（最快）。
  C-u：提示输入渲染宽度，强制重新渲染并复制。"
  (interactive
   (list (when current-prefix-arg
           (read-number "Render width (px): " (car (pdf-view-image-size))))))
  (unless (derived-mode-p 'pdf-view-mode) (user-error "Not in pdf-view-mode"))
  (require 'pdf-cache)
  (let* ((img (and (not force-width) (pdf-view-current-image)))
         (data (or (and img (plist-get (cdr img) :data))
                   (pdf-cache-renderpage (pdf-view-current-page) (or force-width (car (pdf-view-image-size)))))))
    (unless (and (stringp data) (> (length data) 0)) (user-error "No image data"))
    (my/pdf--clipboard-send-bytes data)
    (message "Copied page image to clipboard")))

(after! pdf-view
  (map! :map pdf-view-mode-map
        :localleader
        "y" #'my/pdf-view-copy-page-image-to-clipboard))
#+end_src
