...
(defun my/open-at-point ()
  "Open current file at current line with external editor."
  (interactive)
  (when-let ((file (buffer-file-name))
             (line (line-number-at-pos)))
    ;; Helix
    (start-process "helix" nil "hx" (format "%s:%d" file line))
    ;; 或者 VSCode
    ;; (start-process "code" nil "code" "-g" (format "%s:%d" file line))
    ;; 或者 Vim
