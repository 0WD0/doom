...
** org-src tree-sitter 高亮

#+begin_src emacs-lisp :tangle no
;; 在 org-mode buffer 中启用 tree-sitter 高亮代码块
(after! org
  (setq org-src-fontify-natively t
        org-src-tab-acts-natively t)
  
  ;; 确保相关语言模式启用 tree-sitter
  (defun my/enable-tree-sitter-for-mode ()
    "Enable tree-sitter for supported modes used in org src blocks"
    (when (and (featurep 'tree-sitter)
               (fboundp 'tree-sitter-mode)
               (fboundp 'tree-sitter-hl-mode))
      (tree-sitter-mode 1)
      (tree-sitter-hl-mode 1)))
  
  ;; 为常用的编程语言模式添加 tree-sitter 支持
  (dolist (mode '(c-mode-hook c++-mode-hook python-mode-hook
                  rust-mode-hook js-mode-hook typescript-mode-hook
                  emacs-lisp-mode-hook go-mode-hook java-mode-hook))
    (add-hook mode #'my/enable-tree-sitter-for-mode))
  
  ;; 强制刷新 org buffer 的 fontification
  (defun my/org-restart-font-lock ()
    "Restart font-lock in org mode to apply tree-sitter highlighting"
    (when (eq major-mode 'org-mode)
      (org-restart-font-lock)))
  
  ;; 在 org-mode 启动时应用
  (add-hook 'org-mode-hook #'my/org-restart-font-lock)
